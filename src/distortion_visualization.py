...
while True:
    # 3D -> 2D 투영 시 렌즈 왜곡(dist_coeff) 적용
    dist_pts, _ = cv2.projectPoints(obj_pts, np.zeros(3), np.zeros(3), K, dist_coeff)
    zero_pts, _ = cv2.projectPoints(obj_pts, np.zeros(3), np.zeros(3), K, np.zeros(4))

    img_vector = np.full((img_h, img_w, 3), 255, dtype=np.uint8)
    for zero_pt, dist_pt in zip(zero_pts, dist_pts):
        cv2.line(img_vector, np.int32(zero_pt.flatten()), np.int32(dist_pt.flatten()), (255,0,0))
    
    for pt in dist_pts:
        cv2.circle(img_vector, np.int32(pt.flatten()), 2, (0,0,255), -1)
    
    img_grid = np.full((img_h, img_w, 3), 255, dtype=np.uint8)
    dist_pts = dist_pts.reshape(len(range(*grid_y)), -1, 2)
    for pts in dist_pts:
        cv2.polylines(img_grid, [np.int32(pts)], False, (0,0,255))
    ...
    cv2.putText(merge, info, (10,25), cv2.FONT_HERSHEY_DUPLEX, 0.6, (0,0,0))
    cv2.imshow('Geometric Distortion Visualization: Vectors | Grids', merge)
    ...
    if key == 27: # ESC
        ...
        dist_coeff[0] += 0.05 # k1
    elif key == ord('-') or key == ord('_'):
        dist_coeff[0] -= 0.05 # k1
    elif key == ord(']') or key == ord('}'):
        dist_coeff[1] += 0.05 # k2
    elif key == ord('[') or key == ord('{'):
        dist_coeff[1] -= 0.05 # k2
    elif key == ord('"') or key == ord("'"):
        dist_coeff[2] += 0.01 # p1
    elif key == ord(':') or key == ord(';'):
        dist_coeff[2] -= 0.01 # p1
    elif key == ord('>') or key == ord('.'):
        dist_coeff[3] += 0.01 # p2
    elif key == ord('<') or key == ord(','):
        dist_coeff[3] -= 0.01 # p2
