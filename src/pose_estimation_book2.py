"""
K 초기값이 주어지고
인라인 점만 사용해 K와 distortioin까지 다시 최적화
"""
import numpy as np
import cv2
...
while True:
    ...
    if inlier_num > min_inlier_num:
        ...
        ret, K, dist_coeff, rvecs, tvecs = cv2.calibrateCamera([obj_pts[inliers]], [img_pts[inliers]], (img.shape[1], img.shape[0]), K, dist_coeff, None, None, calib_param)
